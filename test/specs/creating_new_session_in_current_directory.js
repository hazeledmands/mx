...
test('creating new session in current directory', function(t) {
  ...
  harness("horse_js", function(err, stdout, stderr) {
    ...
    t.equal(output[0],
      'mx> tmux -2 new-session -c ' + currentDir + ' -s horse_js -n editor -d',
      'mx should tell tmux to create a new session from the current directory'
    );
    ...
    t.equal(output[3],
      'mx> tmux -2 select-window -t horse_js:1',
      'mx should tell tmux to select the editor window'
    );

    t.equal(output[4],
      'mx> tmux -2 switch-client -t horse_js',
