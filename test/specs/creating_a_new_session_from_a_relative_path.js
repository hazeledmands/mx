...
var CWD = path.resolve(__dirname, '..', 'mocks');
var SESSION = path.resolve(CWD, 'red_sofa_project');

test('creating a new session from a relative path', function(t) {
  t.plan(8);
  harness({cwd: CWD, args: 'red_sofa_project'}, function(err, stdout, stderr) {
    ...
    t.equal(output[0],
      'mocks> tmux -2 new-session -c ' + SESSION + ' -s red_sofa_project -n editor -d',
    ...
    t.equal(output[2],
      'mocks> tmux -2 new-window -c ' + SESSION + ' -n shell -t red_sofa_project',
      'mx should tell tmux to open a new window'
    );

    t.equal(output[3],
      'mocks> tmux -2 select-window -t red_sofa_project:1',
      'mx should tell tmux to select the editor window'
    );

    t.equal(output[4],
      'mocks> tmux -2 switch-client -t red_sofa_project',
